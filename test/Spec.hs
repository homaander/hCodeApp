...
main = do
  ...
  test (showHCode v) "C2D3E4"

  -- HData
  --   Получение данных из числа
  test (showHCode $ toHData 10 1234) "1234"
  test (showHCode $ toHData 16 1234) "4D2"


  -- Code
  --   Кодирование 1 раз
  test (showHCode $ code v) "6B6B6C"
  --   Кодирование N раз
  test (showHCode $ codeN 66 v) "584C32"
  test (showHCode $ v -^> 66) "584C32"

  --   Кодирование через пресет
  let preset66 = getPreset 16 6 66
  test (showHCode $ runPreset preset66 v) "584C32"


  --   Декодировать 1 раз
  test (showHCode $ decode v) "425240"
  --   Декодировать N раз
  test (showHCode $ decodeN 66 v) "323BB5"
  test (showHCode $ v <^- 66) "323BB5"

  --   Декодирование через пресет
  let preset_66 = getPreset 16 6 (-66)
  test (showHCode $ runPreset preset_66 v) "323BB5"

  -- Tape
  --   Основные данные заданного кода в ленте
  test (show $ toTape v) (show $ HTape {tapeId = [HN 16 0,HN 16 0,HN 16 7,HN 16 4,HN 16 5,HN 16 4], tapeOffset = 275, tapeAntiOffset = 229, tapeLength = 504})
